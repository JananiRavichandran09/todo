...
const App = () => {
  ...
  return (
    <div>
      <Input todoValue={todoValue} setTodoValue={setTodoValue} addTodo={addTodo} />
      ...
      <div class=" container">
        <div class="row row-cols-1 row-cols-md-3 row-cols-sm-2 mt-1 p-3 g-4">
          {filteredTodo.map((item, index) => {
            return (
              <Display item={item} index={index} deleteButton={deleteButton} statusUpdate={statusUpdate} handleEdit={handleEdit} />
            )
          })}
        </div>
      </div>
